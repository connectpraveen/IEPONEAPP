...
export class LoginComponent implements OnInit {
  ...
  signInWithEmail() {
    document.getElementById('loginButton').innerText = "Login progress.."
    if (this.user.email.endsWith('@gmail.com')) {
      this.message = "Please login with google";
    }
    else {
      this.logSer.getAccountHolderAssociationType(this.user.email).subscribe((data: any) => {
        localStorage.setItem("association_type", data.associationType);
        ...
        this.authService.signIn(this.user.email, this.user.password)
          ...
          .catch((error) => {
            document.getElementById('loginButton').innerText = "Login with Email";
      ...
      this.accser.saveAccountDataEmailServlet(this.afAuth.auth.currentUser.uid, this.afAuth.auth.currentUser.email, '', ip, 'Firebase')
        .subscribe((data: any) => {          
          ...
          this.logSer.getAccountHolderAssociationType(this.afAuth.auth.currentUser.email).subscribe((data: any) => {
            ...
            this.logSer.addAccountHolders(data.id, this.afAuth.auth.currentUser.email)
              .subscribe((data: any) => { 
                this.logSer.getAccountHolderAssociationType(this.afAuth.auth.currentUser.email).subscribe((data: any) => {
                  localStorage.setItem("association_type", data.associationType);
                  localStorage.setItem("phonelogin", "0");
